...
func Example_createEventNtupleNoSplit() {
	...
	func() {
		...
		var (
			evt   Event
			wvars = rtree.WriteVarsFromStruct(&evt, rtree.WithTitle("evt"), rtree.WithSplitLevel(0))
		)
...
func Example_createEventNtupleSplitLvl1() {
	type P4 struct {
		Px float64 `groot:"px"`
		Py float64 `groot:"py"`
		Pz float64 `groot:"pz"`
	...
	const (
		fname = "../testdata/groot-event-ntuple-split-lvl-1.root"
		nevts = 5
	)
	...
	func() {
		...
		var (
			evt   Event
			wvars = rtree.WriteVarsFromStruct(&evt, rtree.WithSplitLevel(1))
		)
	...
	// -- filled tree with 5 entries
	// -- read back ROOT file
	// === [../testdata/groot-event-ntuple-split-lvl-1.root] ===
	// version: 62400
	//   TTree mytree             (entries=5)
	//     i32 "i32/I"    TBranch
	//     f64 "f64/D"    TBranch
