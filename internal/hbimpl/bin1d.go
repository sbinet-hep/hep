...
func (p Bin1Ds) Less(i, j int) bool { return p[i].XXX_xrange.Min < p[j].XXX_xrange.Min }
...
func (p Bin1Ds) IndexOf(v float64) int {
	i := sort.Search(len(p), func(i int) bool { return v < p[i].XXX_xrange.Max })
	if i == len(p) {
		return OverflowBin
	}
	rng := p[i].XXX_xrange
