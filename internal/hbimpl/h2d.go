...
func NewH2D(nx int, xlow, xhigh float64, ny int, ylow, yhigh float64) *H2D {
	return &H2D{
		XXX_bng: newBinning2D(nx, xlow, xhigh, ny, ylow, yhigh),
...
func NewH2DFromEdges(xedges []float64, yedges []float64) *H2D {
	return &H2D{
		XXX_bng: newBinning2DFromEdges(xedges, yedges),
		XXX_ann: make(Annotation),
	}
}
...
func (g h2dGridXYZ) Dims() (c, r int) {
	return g.h.XXX_bng.XXX_nx, g.h.XXX_bng.XXX_ny
}

func (g h2dGridXYZ) Z(c, r int) float64 {
	idx := r*g.h.XXX_bng.XXX_nx + c
	return g.h.XXX_bng.XXX_bins[idx].SumW()
}

func (g h2dGridXYZ) X(c int) float64 {
	return g.h.XXX_bng.XXX_bins[c].XMid()
}

func (g h2dGridXYZ) Y(r int) float64 {
	idx := r * g.h.XXX_bng.XXX_nx
	return g.h.XXX_bng.XXX_bins[idx].YMid()
}
...
func (h *H2D) MarshalYODA() ([]byte, error) {
	...
	for ix := 0; ix < h.XXX_bng.XXX_nx; ix++ {
		for iy := 0; iy < h.XXX_bng.XXX_ny; iy++ {
			bin := h.XXX_bng.XXX_bins[iy*h.XXX_bng.XXX_nx+ix]
			d := bin.XXX_dist
			fmt.Fprintf(
				buf,
				"%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e\t%d\n",
				bin.XXX_xrange.Min, bin.XXX_xrange.Max, bin.XXX_yrange.Min, bin.XXX_yrange.Max,
				d.SumW(), d.SumW2(), d.SumWX(), d.SumWX2(), d.SumWY(), d.SumWY2(), d.sumWXY, d.Entries(),
			)
		}
	}
...
scanLoop:
	for s.Scan() {
		...
		case !ctx.dist && bytes.HasPrefix(buf, []byte("Total   \t")):
			...
			_, err = fmt.Fscanf(
				rbuf,
				"Total   \tTotal   \t%e\t%e\t%e\t%e\t%e\t%e\t%e\t%d\n",
				&d.x.dist.XXX_sumW, &d.x.dist.XXX_sumW2,
				&d.x.sumWX, &d.x.sumWX2,
				&d.y.sumWX, &d.y.sumWX2,
				&d.sumWXY, &d.x.dist.XXX_n,
			)
			if err != nil {
		...
		case ctx.bins:
			...
			yset[bin.XXX_yrange.Min] = 1
			xmin = math.Min(xmin, bin.XXX_xrange.Min)
			xmax = math.Max(xmax, bin.XXX_xrange.Max)
			ymin = math.Min(ymin, bin.XXX_yrange.Min)
			ymax = math.Max(ymax, bin.XXX_yrange.Max)
	...
	h.XXX_bng = newBinning2D(len(xset), xmin, xmax, len(yset), ymin, ymax)
	h.XXX_bng.XXX_dist = dist
	// YODA bins are transposed wrt ours
	for ix := 0; ix < h.XXX_bng.XXX_nx; ix++ {
		for iy := 0; iy < h.XXX_bng.XXX_ny; iy++ {
			h.XXX_bng.XXX_bins[iy*h.XXX_bng.XXX_nx+ix] = bins[ix*h.XXX_bng.XXX_ny+iy]
		}
	}
	return err
}
