...
func NewS2DFrom(x, y []float64) *S2D {
	...
	s := &S2D{
		XXX_pts: make([]Point2D, len(x)),
		XXX_ann: make(Annotation),
	}
	for i := range s.XXX_pts {
		pt := &s.XXX_pts[i]
		pt.X = x[i]
		pt.Y = y[i]
	}
...
func NewS2DFromH1D(h *H1D, opts ...S2DOpts) *S2D {
	...
	// YODA support
	if _, ok := s.XXX_ann["Type"]; ok {
		s.XXX_ann["Type"] = "Scatter2D"
	}
...
func (s *S2D) Fill(pts ...Point2D) {
	...
	i := len(s.XXX_pts)
	s.XXX_pts = append(s.XXX_pts, make([]Point2D, len(pts))...)
	copy(s.XXX_pts[i:], pts)
}

// Sort sorts the data points by x,y and x-err,y-err.
func (s *S2D) Sort() {
	sort.Sort(points2D(s.XXX_pts))
}
...
func (s *S2D) annToYODA() Annotation {
	ann := make(Annotation, len(s.XXX_ann))
	ann["Type"] = "Scatter2D"
	ann["Path"] = "/" + s.Name()
	ann["Title"] = ""
	for k, v := range s.XXX_ann {
		if k == "name" {
			continue
		}
		ann[k] = v
...
func (s *S2D) annFromYODA(ann Annotation) {
	if len(s.XXX_ann) == 0 {
		s.XXX_ann = make(Annotation, len(ann))
	}
	for k, v := range ann {
		switch k {
		case "Type":
			// noop
		case "Path":
			s.XXX_ann["name"] = string(v.(string)[1:]) // skip leading '/'
		default:
			s.XXX_ann[k] = v
		}
	}
