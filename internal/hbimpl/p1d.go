...
func NewP1D(n int, xmin, xmax float64) *P1D {
	return &P1D{
		XXX_bng: newBinningP1D(n, xmin, xmax),
...
func NewP1DFromH1D(h *H1D) *P1D {
	return &P1D{
		XXX_bng: newBinningP1D(len(h.Binning().Bins()), h.XMin(), h.XMax()),
		XXX_ann: make(Annotation),
	}
}
...
func (p *P1D) annToYODA() Annotation {
	ann := make(Annotation, len(p.XXX_ann))
	ann["Type"] = "Profile1D"
	ann["Path"] = "/" + p.Name()
	ann["Title"] = ""
	for k, v := range p.XXX_ann {
		if k == "name" {
			continue
		}
		ann[k] = v
...
func (p *P1D) annFromYODA(ann Annotation) {
	if len(p.XXX_ann) == 0 {
		p.XXX_ann = make(Annotation, len(ann))
	}
	for k, v := range ann {
		switch k {
		case "Type":
			// noop
		case "Path":
			p.XXX_ann["name"] = string(v.(string)[1:]) // skip leading '/'
		default:
			p.XXX_ann[k] = v
		}
	}
...
func (p *P1D) MarshalYODA() ([]byte, error) {
	...
	for _, bin := range p.XXX_bng.bins {
		d := bin.dist
		fmt.Fprintf(
			buf,
			"%e\t%e\t%e\t%e\t%e\t%e\t%e\t%e\t%d\n",
...
scanLoop:
	...
	p.XXX_bng = newBinningP1D(len(xset), xmin, xmax)
	p.XXX_bng.dist = dist
	p.XXX_bng.bins = bins
	p.XXX_bng.outflows = oflows
	return err
}
