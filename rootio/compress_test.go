...
func TestCompress(t *testing.T) {
	dir, err := ioutil.TempDir("", "rootio-compress-")
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, tc := range []struct {
		...
		for k, want := range wants {
			...
			t.Run(tname, func(t *testing.T) {
				...
				if got, want := str.String(), want.(ObjString).String(); got != want {
				...
				cmd := exec.Command(cxxROOT, "-b", fmt.Sprintf("%s(%q, %d)", macro, fname, len(want.(ObjString).String())))
				cmd.Stdout = buf
				cmd.Stderr = buf
				err = cmd.Run()
				if err != nil {
